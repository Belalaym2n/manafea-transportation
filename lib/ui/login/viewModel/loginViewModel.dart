...
class LoginViewModel extends BaseViewModel<LoginConnector> {
  ...
  Future<void> sendVerification({required String number}) async {
    try {
      ...
      await _otpRepo.sendOTP(phoneNumber: validNumber);
        await SharedPreferencesHelper.saveData(key: "phoneNumber", value: validNumber);
      setLoading(false);
       connector!.navigateToVerify();
    } catch (e) {
      setLoading(false);
      connector?.onError("❌ Failed to send OTP Please Try Again: ${e.toString()}");
    } finally {
      setLoading(false);
     }
  }
  ...
  String? validateSaudiNumber({required String number}) {
    ...
    if (regex.hasMatch(number)) {
      ...
      connector!.onError("Please enter a valid Saudi phone number like: 0501234567 "); // الرقم غير صحيح
      return null;
     }
  }
